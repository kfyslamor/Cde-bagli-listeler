...
}*head,*temp,* deleteNode,*after,*before; // head = her zaman icin baslangici gösterecek olan "dügüm" pointeri.
...
void deleteTheMidNode(int n){
  ...
  if(temp == NULL){
    ...
    for(int i = 0;i<(n/2)-1;i++){//ortadaki nodedan bi öncesini gösterecek.
      temp = temp->next;
    }
